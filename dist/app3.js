(() => {
  ...
  function renderEditor() {
    ...
    host.innerHTML = `<div class="service-heading"><div><h3>${esc(service.name)} 입력</h3><p>메뉴를 설정한 뒤, 담당자가 불러주는 순서대로 갤리별 수량을 입력하세요.</p></div><div class="menu-actions"><button class="small-button" id="add-menu" type="button" ${disabled}>＋ 메뉴 추가</button></div></div>
      <section class="special-requests"><div class="section-title"><div><span>스페셜밀 신청자</span><small>신청한 존으로 정확히 배분</small></div></div><div class="special-request-grid">
